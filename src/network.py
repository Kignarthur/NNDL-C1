...
class Network:
    ...
    def __init__(self, layers: list[int]):
        ...
        # Biases:  Vector with a single column where each line has the bias of a single node.
        # Weights: Matrix where the columns are the nodes and the lines are the weights of a single node.
    ...
    def feedforward_training_matrix(self, A: np.ndarray):
        ...
        for W, b in zip(self.weights, self.biases):
            B = np.repeat(b, batch_size, axis=1)
